...
class KeyInfo:
    def __init__(self, name: str, desc: str, activated: bool, retail: bool):
        self.name = name
        self.desc = desc
...
class KeyList:
    ...
    def add_key(self, key: KeyInfo):
        if self.get_by_name(key.name) is not None:
            return
        self.keys.append(key)

    def get_by_name(self, name: str) -> Optional[KeyInfo]:
        for key in self.keys:
            if key.name == name:
                return key
        return None

    def get_by_desc(self, desc: str) -> Optional[KeyInfo]:
        for key in self.keys:
            if key.desc == desc:
                return key
        return None
    ...
    def get_desc_list(self) -> list[str]:
        return [key.desc for key in self.keys]
